...
**Windows 管理下 Chrome デーモンの信頼性**

ソース実行時の管理下 Chrome パスでは、追加で `PATH` 上の Node.js 22+ およびインストール済みの Google Chrome が必要です。Windows ランタイムバンドルには独自の Node 実行可能ファイルとライセンス、ログインワーカー、およびネイティブプロセススーパーバイザーが含まれており、これらのバンドルを使用する場合はシステムへの Node のインストールは不要です。Windows バンドルのビルドには、Node ディストリビューションの `LICENSE`（`node.exe` の横、そのインストールルート、またはリポジトリ内のコピー）と Windows .NET Framework C# コンパイラが必要です。ブラウザーの起動時にランタイムのダウンロードやインストールは試行されません。

ブラウザー helper とその子孫プロセスは Windows Job Object に属します。helper、そのスーパーバイザー、または所有デーモンを終了すると、所有するツリー全体が終了します。ログインワーカーの実行期限はデフォルトで 60 秒（明示的なログインタイムアウトが優先）、インタラクティブログインのデフォルト期限は 10 分です。Helper ターンは設定された期限または 30 分のトランスポート上限を使用します。停止したシャットダウンは強制的に 2 秒以内に制限されます。バックグラウンドログイン検証はヘッドレスであり、自動的なヘッドありへのフォールバックはありません。ログイン抽出はログインブラウザーが閉じた後にのみ所有プロファイルを開きます。プロファイルのコピーや他プロセスのロック解除は行いません。ブラウザーの識別には実行中の Chrome バージョンが使用され、Chrome 134 に固定されません。プロトコルの stdout は JSON 専用であり、コンソール診断は stderr に出力されます。

ネイティブ回帰テスト（意図的な子/親プロセスの終了や空の `PATH` でのパッケージ版 Node 解決を含む）：
...
変更履歴およびスコープ内のレビュー結果は
[Windows デーモンの修正 PR](https://github.com/Wladefant/codex-chatgpt-web/pull/1) に記録されています。
