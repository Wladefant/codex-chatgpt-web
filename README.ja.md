...
ソース実行時の管理下 Chrome パスでは、追加で `PATH` 上の Node.js 22+ およびインストール済みの Google Chrome が必要です。Windows ランタイムバンドルには独自の Node 実行可能ファイルとライセンス、ログインワーカー、およびネイティブプロセススーパーバイザーが含まれており、これらのバンドルを使用する場合はシステムへの Node のインストールは不要です。Windows バンドルのビルドには、`node.exe` の横にある Node ディストリビューションの `LICENSE` と Windows .NET Framework C# コンパイラが必要です。ブラウザーの起動時にランタイムのダウンロードやインストールは試行されません。
