...
test("daemon streams browser lifecycle through the real helper process", async () => {
  ...
  const config: ResolvedBrowserConfig = {
    ...
    storageStatePath: join(root, "unused-state.json"), imageOutputPath: join(root, "unused-images"),
...
test("accepted compaction retires through the helper as completed without hiding cancellations or errors", async () => {
  ...
  const client = new LauncherBrowserHelperClient({
    ...
    storageStatePath: join(root, "unused-state.json"), imageOutputPath: join(root, "unused-images"), chromeExecutablePath: join(root, "unused-chrome"),
...
test("launcher helper protocol preserves multipart context and the compaction flag", async () => {
  ...
  const client = new LauncherBrowserHelperClient({
    ...
    storageStatePath: "/durable/unused-state.json", imageOutputPath: "/durable/unused-images",
...
test("an older helper cannot silently drop selected skill files and releases the prepared turn", async () => {
  const client = new LauncherBrowserHelperClient({
    ...
    storageStatePath: "/durable/unused.json", imageOutputPath: "/durable/unused-images", chromeExecutablePath: "/durable/chrome", headed: true, autoApproveToolCalls: false,
  });
  const internal = client as unknown as {
    child: unknown;
    ensureChild(): Promise<void>;
